...
public class BoardDAO {
	...
	//검색 기능 쿼리 추가(T:title,W:writer,C:content)
	...
	public void deleteBoard(BoardDTO dto) {
		...
		try {
			...
			pstmt.executeUpdate();	//insert, update, deleye
	...
	public BoardDTO getBoard (BoardDTO dto) {
		...
		try {
			conn=JDBCUtil.getConnection();
			pstmt=conn.prepareStatement(BOARD_GET); 	//글 상세 페이지
			pstmt.setInt(1, dto.getSeq());
			...
			while(rs.next()) {
				...
				board.setWrite(rs.getString("WRITER"));
				board.setContent(rs.getString("CONTENT"));
				board.setRegdate(rs.getDate("REGDATE"));
				board.setCnt(rs.getInt("CNT"));
				
			}
			System.out.println("Board 테이블에서 상세 레코드가 잘 처리 되었습니다.");
			
		}catch(Exception e) {
			e.printStackTrace();
			System.out.println("Board 테이블에서 상세 레코드가 실패 되었습니다.");
		}finally {
	...
	//BOARD_LIST_T="select * from board where TITLE like '%'||?||'%'";
	//BOARD_LIST_W="select * from board where WRITER like '%'||?||'%'";
	 //BOARD_LIST_C="select * from board where CONTENT like '%'||?||'%'";
	//BOARD_LIST_R = "select * from board where REGDATE like '%'||?||'%'";
	public List<BoardDTO> getBoardList(BoardDTO dto){
		...
		try {
			...
			System.out.println("====BoardDAO에서 출력 =========");
