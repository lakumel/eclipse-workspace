...
public class MyController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ( path.equals("/login.do")) {       
			...
			//1.client form 에서 넘어오는 변수 : id, password
			...
			//users : null 경우 <== 해당 ID와 password가 일치하지 않는 경우
			//users : null 이 아닐 경우 	<== 해당 ID와 password가 DB에 존재함.		<===SESSION에 변수에 정보값을 입력
			...
			if(users==null) {	//인증 실패
				...
				//Session 변수를 생성하고 변수에 ID값을 담아서 클라이언트 뷰 페이지로 전송
			...
			//Sptring 에서는 자동으로 Framework에서 자동으로 처리됨
			...
			//4.session 에 값을 저장후 뷰 페이지로 전달 : Model
				//현재 클라이언트가 서버에 접속한 세션을 가지고 옵니다.
			
			//세션 변수 선언
			HttpSession session = request.getSession();
			
			//세션에 변수에 DB에서 select 한 DTO를 저장후 뷰 페이지로 전송
			...
			//클라이언트에서 받은 검색어를 DTO에 검색후 메소드 호출
			String searchCondition = request.getParameter("searchCondition");
			String searchkeyword = request.getParameter("searchkeyword");
			
			//Null을 처리 해야 함. : 주의 :url에서 요청을 했을때 null이 적용됨
			...
			System.out.println(searchkeyword);
				...
				//boardList 에는 board 테이블의 각 레코드를 dto에 저장후 boardList에 추가된 객체를 리턴
				boardList = dao.getBoardList(dto);
				
				
				//리턴받은 boardList를 Client View 페이지로 전송 , (Session에 리스트를 저장후 클라이언트로 전송)
			...
			System.out.println("insertUsers.do를 요청 했습니다.");
			...
			System.out.println("getUsersList.do를 요청 했습니다.");
			...
			System.out.println("getUsers.do를 요청 했습니다.");
