	...
	public static void main(String[] args) {
		/*
		 	List<E> : 인터페이스, 메소드만 선언되어 있다. 구현되어 있지 않다.
		 		: 방번호[index]를 가지고 있다. 방번호에 따라 중복된 값을 저장 할 수 있다.
		 		: 컬렉션 값을 추가하거나, 제거하거나, 수정하거나 할때 메소드를 사용함. 
		 		
		 		-- ArrayList<E>	: 싱글 쓰레드 환경에서 사용
		 		-- Vector<E>	: 멀티 쓰레드 환경에서 사용, 모든 메소드가 동기화 처리가 되어 있기 때문에 멀티 쓰레드 환경에 유리.
		 			--	중간에 어떤 값을 넣거나 제거할 경우 부화가 많이 발생한다.
		 			--	빈번하게 값을 수정하는 경우 부화가 많이 발생된다.
		 		-- LinkedList<E>: 
		 			-- 중간의 어떤 값을 추가/제거 하더라도 부하가 많이 발생되지 않는다.
		...
		//자식의 구현 클래스를 객체화 해서 List 타입으로 선언 : 인터페이스의 메소드를 호출 하면 오버라이딩된 메소드 호출
		...
		//3.addAll(다른 리스트객체) : 자신의 마지막 방에서 다른 컬렉션 객체의 값을 모두 추가할 수 가 있다.
		...
		//삭제 
		//6.remove(int index)	: index : 방번호의 값을 삭제 , 삭제후 뒤의 값이 한칸씩 앞으로 모이게 된다.
		aList3.remove(2);
		...
		//9.isEmpty() : 리스트에 값이 존재하지 않을 경우 true를 리턴, 존재하는 경우 false
		...
		//리스트의 값을 배열로 변환
		//12.toArray() : 리스트에 저장된 값을 배열로 변환 , 리턴 타입이 object이기 때문에 다운캐스팅 해서 처리
		Object[] obj = aList2.toArray();	//
		
		System.out.println(Arrays.toString(obj));
		
		//13.toArray(T[] t) :   리스트를 배열로 끄집어 낼때 사용, T[]		<== 리스트를 배열로 끄집어 낼때 해당 타입의 배열로 바로 끄집어 낸다.
