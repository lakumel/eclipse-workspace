...
public class Student_Test {
	public static void main(String[] args) {
		...
		Scanner sc = new Scanner(System.in); 	//콘솔로 부터 값을 인풋받는 스캐너
		...
		while (run) {
			...
			System.out.println("===========================================");
			System.out.println("위의 변호중 하나의 정수를 입력하세요. >>>>>>");
			
			int selectNo=sc.nextInt();
			if(selectNo == 1) {
				//스캐너로 학생수를 인풋 받아서 StudentNum변수에 할당합니다.
				System.out.println("1.원하는 학생 수를 입력하세요>>>>>");
				...
				if(studentNum == 0) {
					...
					//학생수에대한 각각의 student 객체를 생성후 점수를 스캐너로 입력 받고 aList에 저장합니다.
					for(int i = 1; i <= studentNum; i++) {
						
						Student student = new Student();	//Student 객체 생성
						System.out.println(i + " 번째 학생의 점수를 입력해 주세요>>>");
						int score = sc.nextInt();	//스캐너로 정수를 인풋 받도록 함.
				...
				System.out.println("[3.점수 리스트]를 출력 합니다.");
				...
				//aList에 Student 객체에 getScore()를 사용해서 점수를 끄집어 내면서 처리
				for(int i =0; i<aList.size(); i++) {
					...
					//최대 점수를 maxScore 변수에 할당 : if조건으로 처리, 삼항 연산자로 처리
					if(score > maxScore) {
						maxScore = score;
					}
				}
			...
			run =false;
				...
				System.out.println("잘못된 번호를 입력했습니다. 1 ~ 5번까지만 선택 하세요");
				System.out.println();
			}
