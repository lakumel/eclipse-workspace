...
class A{		//data 필드에 들어오는 값으로 각 객체를 식별하기 위해서는 : hashcode(), equals() 가 재정의 되어 있어야 set에 넣을때 
	int data;
...
class B{
	...
	//1.equals()를 재정의 : set 에 중복된 값을 넣지 않도록 식별자를 생성,  object의 equals() 는 stack의 주소 비교, ==
		//data필드의 Heap의 값을 비교하도록 재정의
	@Override
	public boolean equals(Object obj) {		//b1.equals(b2)
		if(obj instanceof B) {		//다운 캐스팅 하기 전에 해당 타입이 존재하는지 확인후 처리
			if(this.data == ((B)obj).data) {	//Heap에 저장된 data필드의 값을 비교
				return true;
			}else {
				return false;
			}
	...
	public int hashCode() {
		...
		return Objects.hash(data); 	//data필드의 값으로 hashcode를 생성함. 16진수
	}
	...
	public static void main(String[] args) {
		...
		/*
		 	set<E> : 1.방번호[index]가 존재하지 않는다. 2.중복된 값을 넣을 수 없다.
		 	 	...
		 	 	- 자신이 생성한 특정한 클래스(Type)를 set의 넣을 경우 : Object에 있는 Hashcode() 메소드와 equals() 메소드를 재정의 해야 함.
		...
		System.out.println(hset1); //[33, 22]
