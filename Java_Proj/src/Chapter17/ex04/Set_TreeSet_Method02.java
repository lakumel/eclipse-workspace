	...
	public static void main(String[] args) {
		/*
		 	TreeSet은 set<E>, sortedSet<E>(정렬), NavigableSet<E>(검색) 인터페이스를 구현한 클래스
		 		-- TreeSet을 객체화 해서 Set 타입으로 지정할 경우 Set의 메소드만 사용가능
		 		-- TreeSet을 객체화 해서 TreeSet타입으로 지정하면 정렬/검색 기능의 메소드를 사용 가능하다.
		...
		//4.higher(E element) : element 보다 큰값을 출력
		...
		//9.sortedSet<E> headSet(E) : element값의 Head 쪽으로 정렬해서 값을 저장 <element는 미포함>
		for(int i = 50; i > 0; i -=2) {
			treeSet.add(i);
		}
		System.out.println(treeSet);
		...
		System.out.println("=============================");
		//10.NavigableSet<E> headSet(E element) : element값의 Head 쪽으로 정렬해서 값을 저장 <element는 포함>
		for(int i = 50; i > 0; i -=2) {
			treeSet.add(i);
		}
		System.out.println(treeSet);
		...
		//Head쪽으로 갈때는 미포함, Tail 쪽으로 갈때는 포함.	 <==다른언어에서 공통적인 상황
		
		//11.sortedSet<E> tailset(E element) : element값의 Tail 쪽으로 정렬해서 값을 저장 <element는 포함>
		...
		//12.NavigableSet<E> tialset(E element) : element값의 Tail 쪽으로 정렬해서 값을 저장 <element는 포함>
		for(int i = 50; i > 0; i -=2) {
			treeSet.add(i);
		}
		System.out.println(treeSet);
