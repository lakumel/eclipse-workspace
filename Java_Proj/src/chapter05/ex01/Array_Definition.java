...
public class Array_Definition {
	public static void main(String[] args) {
		...
		/*
		 자료형: 자바는 변수명 앞에 반드시 자료형이 선언되어야 한다.
		 	-기본 자료형(8) : boolean, 정수형(byte, short, int, long), 실수(float, double), 문자 (char) 
		 		- 변수명과 값이 Stack 메모리 영역에 저장됨
		 		- 사용전에 반드시 초기값이 들어가 있어야 한다./할당 되어 있어야 한다.
		 		- 첫자가 소문자로 되어 있다.
		 		- 		== <==Stack 영역의 값을 비교하는 연산자.
		 		
		 		
		 	-참조 자료형 : 배열/열거   /객체(class)/인터페이스  //참조자료형에 들어가 있는 값은 (Heap) 영역에 저장이 되어 있다.
		 		- 무한대로 만들 수 있다.
		 		- 변수명(Stack) : heap영역의 참조 주소 값이 변수명에 저장되어 있다.
		 		- 값(Heap)
		 		-	참조 자료형에서 == <==Stack 의 값을 비교(Heap의 주소를 비교)
		 		-			heap 값을 비교를 할때는 equals()를 사용해야 한다.
		 		- Heap 영역은 반드시 초기값이 들어가 있어야 한다.
		 		- 변수 선언만 하고 초기값을 넣지 않더라도 강제로 자동으로 할당이 됨.
		 		- boolean : false, 정수[bytem short, int, long] : 0, 실수[float, double] : 0.0 
		 		 
		 	배열 : 하나의 배열 변수에 여러개의 값을 할당할 수 있다.
		 	 	- 반드시 방의 개수를 선언해야 한다.
		 	 	- 한번 방의 갯수가 지정되면 늘리거나 줄일수가 없다.
		 	 	- 배열의 방번호 : index [0, 1, 2, 3, 4]
		 	 	- 배열의 방의 갯수 : 배열변수명.length() 	
		 	 	- 초기값을 설정하지 않더라도 자동으로 기본값이 들어간다. 
		 */
		...
		//1.변수 선언 	: 하나의 값만 저장됨. 변수에 다른 값을 저장 할 수 있다.
			...
			int b;
			b=20;
			b=50; 						//선언 이후에 나중에 값을 넣을 수 있다.
			
		//2.배열 선언 동시에 방의 크기를 지정
		  int[]arr1 = new int [3];		
		  	//int [] : 배열 변수에 저장할 자료형, arr1 : 배열 변수명, 
		  	//new : 변수의 값을 Heap 영역에 저장하겠다.
		  	//int[3] : 배열의 방의 개수 : 3 , index(0, 1, 2)
		  
		//3.배열의 기본값을 출력  : 선언만 해도 강제로 초기값이 Heap 에 들어감
	...
	//	  System.out.println(arr1[3]); 		    //arr1 배열변수의 3번방의 값을 출력
		...
		//4.배열 방의 값을 저장
		  ...
		  System.out.println(arr2[0]);  	//0.0 이 기본값으로 들어가 있다.
		  
		  //배열 변수가 arr2의 각 방에 실수를 저장
		  arr2[0]=10.0; 	
		  arr2[1]=20.0; 	
		  arr2[2]=30.0; 	
		  arr2[3]=40.0; 	
		  arr2[4]=50.0; 	
		  arr2[5]=60.0; 	
		  //arr2[6]=70.0; 	 			ArrayIndexOutOfBound Exception(배열의 방번호를 알 수 없다.)<=======오류가 발생하는 구문 6번방이 없기 떄문
...
//		  System.out.println("arr2의 6번째 방의 값 : " + arr2[6] );
		  ...
		  //1부터 100까지 배열 변수에 저장후 합계값 출력
		  
		  int[] arr3=new int[100];
		 
		  		System.out.println(arr3.length);  			//배열 방의 갯수를 출력
		  
		  //for문을 사용해서 배열변수의 값을 할당.
		  		for(int i =0 ; i<100; i++) {
		  			arr3[i]=i + 1; 			//배열의 각방에 1[0],2[1],3[2],4[3],5[4] 값을 넣는다.
		  									//0번방에 1이 들어가고 1번방에 2가 들어간다. ->arr3[0]=0+1
		  		}
