...
//부모 클래스 (Human) <== 필드와 메소드 : 자식클래스에서 공통된 특징을 갖는 필드와 메소드를 정의한다.
class Human {
	
	String name; //모든 사람은 이름을 가지고 있다.
...
public class Inheritance01 {
	public static void main(String[] args) {
		/*상속(inheritance) : 부모 클래스(객체화)의 필드(속성) 와 메소드(기능), 이너클래스를 자식클래스에서 상속받아서 사용.
		 	- 생성자는 상속되지 않는다.
			- 중복된 코드를 방지 할 수 있다. 코드가 간결해지고 생산성이 향상됨
			- 객체 지향언어의 꽃이다
			- 다형성 : 부모클래스의 내용을 상속받아서 다양한 형태로 출력 : 동물(부모클래스) <== 사람, 독수리, 사자, 호랑이, 개, 고양이
					--부모클래스의 필드와 메소드는 자식 클래스로 내려감.
			- 배열이나 컬렉션에 담아서 사용(부모클래스의 타입으로 업캐스팅해서 배열이나 컬랙션에 저장을 한다.)
				-- 배열이나 컬랙션에 담긴 내용을 다운 캐스팅해서 사용.
			- 자바는 다중상속이 지원되지 않는다.(자식 클래스는 하나의 부모 클래스만 가진다.)
		*/
		...
		s1.studentID = 1111;	//Student 클래스에 있는 필드
