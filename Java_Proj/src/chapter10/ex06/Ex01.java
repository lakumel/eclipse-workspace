...
class Student{
	...
	String name ;
	//합계와 평균을 출력하는 메소드 : 부모클래스의 메소드 : 실행코드가 없다. 자식클래스에서 오버라이딩 해서 구현코드를 생성함
	void sum_avg() {
	}
}
class Student_Sub extends Student{
	...
	int eng;
	int math;
	//1.생성자로 매개변수값을 받아서 각 필드의 값을 할당. name, kor ,eng, math
	...
	void sum_avg() {
		...
		System.out.printf("%s 님의 과목의 국어, 영어, 수학의 합은 : %d 이며, 평균은 :  %.2f 입니다.", name, sum, avg);
		System.out.println();
	}
}
...
public class Ex01 {
	...
	//각 객체의 필드의 값 할당은 생성자 호출시 매개변수로 값을 넣도록 하세요.
		...
		//배열로 변수 선언
		int[] sum = new int[3]; 		//sum[0] : 국어 점수의 합, sum[1] : 영어 점수의 합, sum[2] : 수학 점수의 합
		
		//모든 사용자의 국어점수의 총합 :000
		//모든 사용자의 수학점수의 총합 :000
		//모든 사용자의 영어점수의 총합 :000
		...
		for(int i =0; i<arr.length; i++) {
				...
				sum[1]+=((Student_Sub)arr[i]).eng; 	//sum[0] = sum[0] + eng
				sum[2]+=((Student_Sub)arr[i]).math; //sum[0] = sum[0] + math
		}
