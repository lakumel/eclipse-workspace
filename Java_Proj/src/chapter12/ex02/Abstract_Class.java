...
//2. 익명 클래스로 사용하는 방법 : <<한번 사용하고 더이상 쓰지 않을 경우>>
abstract class C{
	abstract void cry();
}
	...
	public static void main(String[] args) {
		/*
		 	추상 클래스(Abstract Class)
		 		...
		 		- 추상 클래스는 객체화 할 수 없다. 그러나 자식을 객체화 해서 타입으로는 지정이 가능하다.
		 		- 추상 클래스를 작동하는 방법은 2가지
		 			-1.자식 클래스에서 상속을 통해서 추상 메소드를 구현한 클래스를 만들고 (콘크리트 클래스)
		 					자식 클래스를 객체화 해서 사용
		 					<<자주 사용하는 경우>>
		 			-2. 콘크리트 클래스없이 익명 클래스를 만들어서 사용,
		 					<<한번 사용하고 버리는 경우>>, 안드로이드 폰 개발시 이벤트 처리 할 때 많이 사용.
		 */
		
		//1.구현 클래스를 만들고 사용하는 방법(콘크리트 클래스를 객체화 해서 타입을 추상클래스로 지정후 메소드 호출)
		A a1 = new B();
		a1.cry();
