...
class Goods<T>{		//클래스 이름뒤의 <T> : 객체를 생성할떄 지정되는 타입
	private T t;
	...
	public static void main(String[] args) {
		/*
		 	제너릭 클래스 : 다양한 타입을 저장 할 수 있는 클래스
		 		...
		 		- <T>에 들어오는 타입은 반드시 객체형 자료형이여야 한다. 기본 자료형은 올 수 없다. wrapper class를 사용함.
		 			
		 			wrapper class : 기본자료형을 객체형 자료형으로 만듬
		 				 기본 자료형	<===>	객체형 자료형
		 				- boolean 	<===> 	Boolean
		 				- char 	  	<===>  	Charter
		 				- byte 		<===>	Byte
		...
		//getter를 사용해서 Apple 객체를 가져옴.	//자신이 원하는 객체를 주입후 다운 캐스팅 없이 출력
		...
		//Getter를 사용해서 Banana 객체를 가져와서 필드에 내용을 출력 //다운 캐스팅 없이 필드의 내용을 출력
