...
//인터페이스 : 제너릭 타입 제한시 인터페이스일때 다중제한이 가능하다. 클래스 일때는 하나만 처리할 수 있다.
		...
		//인터페이스로 제한을 두면 인터페이스를 구현한 하위의 모든 클래스가 타입으로 지정될 수 있다.
interface IF1{
	void print();	//public abstract 가 생략되어 있다. 1.자식클래스에서 구현 클래스를 만들고 사용, 2. 익명 클래스를 사용해서 구현하는 방법
}
...
public class BoundedTypeOfGenericMethod {
	...
	public static void main(String[] args) {
		/*
		 	제너릭 메소드에서 타입 제한 : 
		 		- 타입제한을 두지 않을때 : 모든 타입이 매개변수 인자로 오기 때문에 object의 메소드만 사용 가능
		 		- 타입을 제한을 두면 : 제한을 둔 클래스의 메소드를 사용가능
		 */
		
		//
		A a = new A();	//일반 클래스 : number로 제한을 두면 String은 올 수가 없다.
		...
		//제너릭 메소드 호출
		b1.<IF1> method1(new Aa());			//자식클래스를 객체화 해서 인터페이스 타입으로 오버라이딩 
		b1.<IF1> method1(new Aaa());
		
		//제너릭 메소드 호출() : 익명 클래스를 사용해서 호출
		//인터페이스는 객체화 할 수 없다. : 자식을 객체화 해서 타입으로는 지정 가능하다.
		//		-- 인터페이스의 메소드 호출시 자식의 오버라이딩 된 메소드 호출
